...
const PPDDualSealsPulpPaper = () => {
  const [activeTab, setActiveTab] = useState(null)
  const [mainImage, setMainImage] = useState("/assets/Ourproducts/Engineeredseals/PPDDualseals/image1.jpg")
  ...
  const productFeatures = [
    ...
    "Double pressure balanced design",
    "Designed with provision for internal barrier fluid circulation",
    "No dynamic O-Ring on the shaft",
    "Robust construction with shrink-fitted Seal faces have a large clearance to the shaftseal faces",
    "Static springs on both the sides",
]

  const technicalFeatures = [
   "Seals can be operated with pressurized buffer fluid or with quench",
   ...
   "Misalignment during installation and operations is reduced due to the static springs on both the faces",
   "Dual seal can also be used as a single seal by removing the atmospheric seal parts"
  ]
  ...
  const materialOptions = ["Seal face: Silicon carbide (Q12), Carbon (A)",
"Secondary seals: FKM (V), EPDM (E), FFKM (K)",
"Metal parts: CrNiMo steel (G), Grade 5A (4T), SMO 654 (4U)"  ]

  const performanceCapabilities = [
   "Temperature: t = -20 °C ... +140 °C (-4 °F ... +284 °F) (180 °C (356 °F) with FFKM elastomers)",
   "Pressure: p1 ... 25 bar (363 PSI), p3",
   "Speed= 20 m/s(66ft/s)",
   "Non-flow operation: Temperature: t = +5 °C ... +100 °C (+41 °F ... +212 °F)",
   "Pressure: p1 max. 10 bar (145 PSI), p3 > p1",
   "Sliding velocity: vg ... 20m/s (66 ft/s)"
  ]

  const partsList = [
    { no: "1", description: "Seal face" },
    { no: "5", description: "Seat" },
    { no: "13", description: "Sleeve" },
    { no: "16", description: "	Housing" },
    { no: "18", description: "Pin" },
    { no: "37", description: "Assembly fixture" },
    { no: "2, 12, 15, 22, 23, 24, 25", description: "O-Ring" },
    { no: "43", description: "Plug" },
  ...
  const thumbnails = [
    "/assets/Ourproducts/Engineeredseals/PPDDualseals/image1.jpg",
    "/assets/Ourproducts/Engineeredseals/PPDDualseals/image2.jpg",
  ]
      ...
      <section className="max-w-[480px] md:max-w-[768px] lg:max-w-[1024px] xl:max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 xl:px-32 py-8 md:py-12">
        <div className="flex flex-col lg:flex-row gap-8 md:gap-16 lg:gap-28">
          ...
          <div className="w-full  lg:pr-8">
            ...
            {/* Details Button */}
            <button className="flex w-full h-12 sm:h-14 p-[10px_24px] font-[Manrope] sm:p-[12px_32px] items-center justify-center gap-2.5 bg-[#CF422A] text-white font-manrope text-lg sm:text-xl font-bold mb-6">
              Details
            </button>
            ...
            <div className="w-full border border-gray-200 overflow-x-auto">
              ...
              {partsList.map((part, index) => (
                <div key={index} className="flex border-b border-gray-200 last:border-b-0">
                  <div className="flex p-[8px_10px] sm:p-[10px_12px] items-start flex-1 border-r border-gray-200 min-w-[100px]">
                    <p className="text-black ffont-[Manrope] text-base sm:text-lg md:text-xl font-normal leading-6 sm:leading-7 tracking-[0.4px] flex-1">
                      {part.no}
                    </p>
                  </div>
          ...
          <div className="w-full lg:w-auto lg:pl-8 mt-8 lg:mt-0">
            ...
            <div className="flex flex-col gap-3 w-full ">
              ...
              <div className="w-full">
                ...
                >
                  <h3 className="font-[Manrope] text-[16px] md:text-[20px] font-bold capitalize">Suitable for Equipment</h3>
...
export default PPDDualSealsPulpPaper
